import { u as J, d as _, r as t, j as e, t as h } from "./index-BBLpVi8g.js";
import { B as w } from "./button-52dtC3UY.js";
import { g as V, m as q } from "./multiplayer-share-url-BDInfhC3.js";
import { S as Q, C, b as E, F as X, r as Z } from "./fighter-game-multiplayer-DYsUuTBf.js";
import { W as ee, C as se } from "./wifi-CShB5cPf.js";
import { c as re, G as te } from "./grid-3x3-DlLMeids.js";
import { R as ae } from "./rotate-ccw-jsALlNl_.js";
import { R as ne, U as le } from "./users-BuTQtawT.js";
...
function we() {
  const R = J(), y = _().roomId, [o, I] = t.useState(null), [m, G] = t.useState(false), [L, S] = t.useState(true), [c, T] = t.useState({ id: "", username: "" }), [x, D] = t.useState(null), [f, O] = t.useState("waiting"), [N, U] = t.useState(0), [ce, H] = t.useState(0), [u, M] = t.useState(false), [b, Y] = t.useState(false), [i, a] = t.useState({ a: { pressed: false }, d: { pressed: false }, w: { pressed: false }, s: { pressed: false }, " ": { pressed: false }, ArrowLeft: { pressed: false }, ArrowRight: { pressed: false }, ArrowUp: { pressed: false }, ArrowDown: { pressed: false } }), l = t.useRef(null), A = t.useRef(null), j = t.useRef(null);
  t.useEffect(() => {
    const { playerId: r, username: s } = V();
    T({ id: r, username: s });
  }, []), t.useEffect(() => {
    if (!y || !c.id) return;
    (async () => {
      try {
        S(true);
        const n = new X(y, c.id, c.username);
        D(n), l.current = n, n.onGameState((p) => {
          const k = p.players.length;
          if (N > k && N > 1) {
            const z = N - k;
            h.warning(`${z} player${z > 1 ? "s" : ""} left the room`);
          }
          const F = JSON.parse(JSON.stringify(p));
          I(F), O(F.status), U(k);
        }), n.onConnection((p) => {
          G(p), p ? h.success("Connected to fighter game!") : h.error("Disconnected from fighter game. You can try refreshing the page to reconnect.");
        }), n.onError((p) => {
          console.error("Game error:", p), h.error(`Game error: ${p.message}`);
        }), await n.connect(), S(false);
      } catch (n) {
        console.error("Failed to initialize game:", n), h.error("Failed to connect to game"), S(false);
      }
    })();
    const s = (n) => {
      l.current && l.current.isConnected() && (n.preventDefault(), n.returnValue = "Are you sure you want to leave the game? You will be disconnected from the room."), l.current && l.current.disconnect();
    }, d = () => {
      l.current && l.current.disconnect();
    };
    return window.addEventListener("beforeunload", s), window.addEventListener("popstate", d), () => {
      window.removeEventListener("beforeunload", s), window.removeEventListener("popstate", d), l.current && (l.current.disconnect(), l.current = null);
    };
  }, [y, c]), t.useEffect(() => () => {
  ...
  t.useEffect(() => (window.addEventListener("keydown", P), window.addEventListener("keyup", v), () => {
    ...
    if (!x || !m) return;
    const r = o == null ? void 0 : o.players.find((n) => n.id === c.id), s = o == null ? void 0 : o.fighters.find((n) => n.id === (r == null ? void 0 : r.fighterId));
    let d;
    s && (o == null ? void 0 : o.fighters.indexOf(s)) === 0 ? d = { left: i.a.pressed, right: i.d.pressed, up: i.w.pressed, down: i.s.pressed, attack1: i[" "].pressed } : d = { left: i.ArrowLeft.pressed, right: i.ArrowRight.pressed, up: i.ArrowUp.pressed, down: i.ArrowDown.pressed, attack1: i.ArrowDown.pressed }, x.sendInput(d);
  }, [i, x, m, o, c.id]), t.useEffect(() => {
    if (!o) return;
    const r = A.current;
    if (!r) return;
    ...
    if (!s) return;
    const d = () => {
      Z(s, o, u), j.current = requestAnimationFrame(d);
    };
    return d(), () => {
  ...
  const W = t.useCallback(() => {
    ...
    try {
      await navigator.clipboard.writeText(q(y, "fighter")), h.success("Shareable link copied to clipboard!");
    } catch {
      h.error("Failed to copy shareable link");
    }
  }, K = () => {
    H((r) => r + 1);
  };
  if (L) return e.jsx("div", { className: "min-h-screen bg-black flex items-center justify-center", style: { fontFamily: "'Press Start 2P', monospace" }, children: e.jsx("div", { className: "bg-gray-900 border-4 border-red-500 rounded-lg p-8", style: { boxShadow: "0 4px 12px rgba(0,0,0,0.8)" }, children: e.jsxs("div", { className: "text-center", children: [e.jsxs("div", { className: "flex items-center justify-center gap-3 mb-6", children: [e.jsx(Q, { className: "h-8 w-8 text-red-500" }), e.jsx("h2", { className: "text-xl font-bold text-red-500", style: { fontSize: "12px", textShadow: "2px 2px 0px #000", fontFamily: "'Press Start 2P', monospace" }, children: "CONNECTING TO FIGHTER GAME..." })] }), e.jsx("div", { className: "animate-spin rounded-full h-12 w-12 border-4 border-red-500 border-t-transparent mx-auto mb-4" }), e.jsxs("p", { className: "text-red-300", style: { fontSize: "8px", fontFamily: "'Press Start 2P', monospace" }, children: ["JOINING ROOM ", y] })] }) }) });
  const B = o == null ? void 0 : o.players.find((r) => r.id === c.id), g = o == null ? void 0 : o.fighters.find((r) => r.id === (B == null ? void 0 : B.fighterId));
  return e.jsxs("div", { className: "flex min-h-screen flex-col bg-black text-white", style: { fontFamily: "'Press Start 2P', monospace" }, children: [e.jsxs("div", { className: "fixed top-4 right-4 z-100 flex gap-2", children: [e.jsx("button", { onClick: () => Y(!b), className: "bg-gray-700 border-2 border-red-500 hover:bg-red-500 hover:text-black text-red-400 px-4 py-2 text-xs transition-colors", style: { fontSize: "8px", fontFamily: "'Press Start 2P', monospace", boxShadow: "0 0 0 2px #000, inset 0 0 0 1px #000" }, "aria-pressed": b, "aria-label": b ? "Hide panel" : "Show panel", children: b ? "HIDE PANEL" : "SHOW PANEL" }), e.jsx("button", { onClick: () => R("/multiplayer"), className: "bg-red-600 border-2 border-red-400 hover:bg-red-500 text-white px-4 py-2 text-xs transition-colors", style: { fontSize: "8px", fontFamily: "'Press Start 2P', monospace", boxShadow: "0 0 0 2px #000, inset 0 0 0 1px #000" }, "aria-label": "Back to multiplayer games", children: "BACK TO MULTIPLAYER GAMES" })] }), e.jsxs("div", { className: `flex min-h-0 flex-1 flex-col pt-16 sm:pt-[4.5rem] ${b ? "lg:flex-row lg:items-stretch lg:gap-4 lg:px-4 lg:pb-4" : "items-center justify-center px-3 pb-6 sm:px-4"}`, children: [e.jsx("div", { className: `flex min-h-0 w-full min-w-0 flex-col items-center justify-center ${b ? "lg:flex-1 lg:py-2" : "flex-1"}`, children: e.jsxs("div", { className: "flex w-full max-w-[min(1024px,calc(100vw-1.5rem))] flex-col overflow-hidden rounded-lg border-4 border-red-500 bg-gray-900", style: { boxShadow: "0 4px 12px rgba(0,0,0,0.8)" }, children: [e.jsx("div", { className: "border-b-4 border-red-500 bg-red-500 px-3 py-3 sm:px-4", children: e.jsxs("div", { className: "flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between", children: [e.jsxs("div", { className: "flex flex-col gap-1 sm:flex-row sm:items-center sm:gap-4", children: [e.jsx("h3", { className: "text-black font-bold shrink-0", style: { fontSize: "10px", fontFamily: "'Press Start 2P', monospace", textShadow: "1px 1px 0px #000" }, children: "GAME ARENA" }), e.jsxs("p", { className: "text-black/90 break-all", style: { fontSize: "7px", fontFamily: "'Press Start 2P', monospace" }, children: ["ROOM ", y] })] }), e.jsxs("div", { className: "flex flex-wrap items-center gap-2 lg:gap-3", children: [e.jsxs("div", { className: "flex items-center gap-1.5", children: [m ? e.jsx(ee, { className: "h-4 w-4 text-black shrink-0" }) : e.jsx(ie, { className: "h-4 w-4 text-black shrink-0" }), e.jsx("span", { className: "text-black font-bold", style: { fontSize: "8px", fontFamily: "'Press Start 2P', monospace" }, children: m ? "CONNECTED" : "DISCONNECTED" })] }), e.jsx("div", { className: `px-3 py-1.5 border-2 text-black ${f === "playing" ? "bg-green-400 border-black" : f === "waiting" ? "bg-yellow-400 border-black" : f === "finished" ? "bg-red-300 border-black" : "bg-gray-300 border-black"}`, style: { fontSize: "8px", fontFamily: "'Press Start 2P', monospace", boxShadow: "0 0 0 2px #000, inset 0 0 0 1px #000" }, children: f === "playing" ? "PLAYING" : f === "waiting" ? "WAITING" : f === "finished" ? "FINISHED" : "\u2014" }), f === "finished" && e.jsxs(w, { type: "button", variant: "outline", size: "sm", onClick: W, disabled: !m, className: "border-2 border-black bg-yellow-400 px-2 py-1.5 font-bold text-black hover:bg-yellow-300 disabled:opacity-50", style: { fontSize: "8px", fontFamily: "'Press Start 2P', monospace", boxShadow: "0 0 0 2px #000, inset 0 0 0 1px #000" }, "aria-label": "Play again in this room", children: [e.jsx(ae, { className: "mr-1.5 inline h-3.5 w-3.5" }), "PLAY AGAIN"] }), e.jsxs(w, { type: "button", variant: "outline", size: "sm", onClick: $, className: "bg-black border-2 border-black hover:bg-gray-900 px-2 py-1.5 text-white", style: { fontSize: "7px", fontFamily: "'Press Start 2P', monospace", boxShadow: "0 0 0 2px #000, inset 0 0 0 1px #333" }, children: [e.jsx(se, { className: "h-3.5 w-3.5 mr-1.5 inline" }), "COPY LINK"] }), e.jsx(w, { type: "button", variant: "outline", size: "sm", onClick: K, className: "bg-gray-800 border-2 border-black text-white hover:bg-gray-700 px-2 py-1.5", style: { fontSize: "8px", fontFamily: "'Press Start 2P', monospace" }, "aria-label": "Refresh view", children: e.jsx(ne, { className: "h-4 w-4" }) }), e.jsxs(w, { variant: u ? "default" : "outline", size: "sm", onClick: () => M(!u), className: `px-3 py-2 border-2 font-bold transition-colors ${u ? "bg-green-400 border-black text-black" : "bg-gray-800 border-black text-white hover:bg-gray-700"}`, style: { fontSize: "8px", fontFamily: "'Press Start 2P', monospace", boxShadow: "0 0 0 2px #000, inset 0 0 0 1px #000" }, children: [e.jsx(te, { className: "h-4 w-4 mr-2" }), "GRID"] })] })] }) }), e.jsx("div", { className: "w-full bg-black", children: e.jsx("div", { className: "relative w-full", style: { aspectRatio: `${C} / ${E}`, maxHeight: "min(576px, calc(100svh - 12rem), calc(100vh - 12rem))" }, children: e.jsx("canvas", { ref: A, width: C, height: E, className: "absolute inset-0 block h-full w-full max-h-full max-w-full", style: { imageRendering: "pixelated" } }) }) })] }) }), b && e.jsxs("div", { className: "mx-auto w-full max-w-lg shrink-0 space-y-4 px-3 pb-4 lg:mx-0 lg:w-80 lg:max-w-sm lg:overflow-y-auto lg:px-0 lg:pb-0", children: [e.jsxs("div", { className: "bg-gray-900 border-4 border-red-500 rounded-lg overflow-hidden", style: { boxShadow: "0 4px 12px rgba(0,0,0,0.8)" }, children: [e.jsx("div", { className: "bg-red-500 px-4 py-3 border-b-4 border-red-500", children: e.jsxs("h3", { className: "flex items-center gap-2 text-black font-bold", style: { fontSize: "10px", fontFamily: "'Press Start 2P', monospace", textShadow: "1px 1px 0px #000" }, children: [e.jsx(le, { className: "h-4 w-4" }), "PLAYERS (", (o == null ? void 0 : o.players.length) || 0, "/2)"] }) }), e.jsx("div", { className: "p-4 space-y-3", children: o == null ? void 0 : o.players.map((r) => {
    const s = o.fighters.find((d) => d.id === r.fighterId);
