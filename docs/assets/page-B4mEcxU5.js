import { u as oe, e as ae, r as p, h as ne, k as le, j as e, t as O } from "./index-oJ3SvOXV.js";
import { C as ce } from "./ChessBoardMultiplayer-DWeR5Sb_.js";
...
import { u as pe, a as me } from "./multiplayer-join-gate-DZ7tIr9u.js";
import "./gin-DHzzR3IR.js";
function ve() {
  var _a;
  const j = oe(), F = ae(), h = pe("checkers"), L = (_a = F.state) == null ? void 0 : _a.roomPassword, [t, $] = p.useState(null), [b, _] = p.useState(false), [K, T] = p.useState(true), [y, V] = p.useState({ id: "", username: "" }), [C, J] = p.useState(null), [f, S] = p.useState(null), [E, P] = p.useState([]), [k, Z] = p.useState(false), [d, Q] = p.useState(null), [g, X] = p.useState(false), G = 360, ee = G * 2 + 24 * 2, m = p.useRef(null), z = (r, a, o) => {
    const n = [], i = [[-1, -1], [-1, 1], [1, -1], [1, 1]];
    for (const [s, l] of i) {
      const c = a.row + 2 * s, x = a.col + 2 * l, w = a.row + s, v = a.col + l;
      if (c >= 0 && c < N && x >= 0 && x < N && !o[c][x]) {
        const B = o[w][v];
        B && B.color !== r.color && n.push({ row: c, col: x });
      }
    }
    return n;
  }, q = (r, a) => {
  ...
  const te = p.useCallback((r, a) => {
    if (!t || !C || !b || t.status !== "playing") return;
    ...
    if (E.some((s) => A(s, o))) {
      const s = [];
      if (Math.abs(r - f.row) === 2) {
        const c = (f.row + r) / 2, x = (f.col + a) / 2;
        s.push({ row: c, col: x });
      }
      const l = { from: f, to: o, ...s.length > 0 && { captures: s } };
      C.sendMove(l), S(null), P([]);
    } else S(null), P([]);
  }, [t, C, b, f, E, d]), M = p.useMemo(() => {
    var _a2;
